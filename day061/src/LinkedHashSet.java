...
/**
 * @author linlin
 * @create 2022-05-14 10:37
 * *  |----Collection接口：单列集合，用来存储一个一个的对象
 * *      |----Set接口：存储无序的、不可重复的数据。 --->高中讲的集合
 * *          |----HashSet:作为Set 接口的主要实现类；线程不安全的；可以存储Null值
 *                 LinkedHashSet：作为HashSet的子类；遍历其内部数据时，可以按照添加的遍历
 *                                  对于频繁遍历的操作LinkedHastSet效率高于HashSet
...
public class LinkedHashSet {
     ...
     *
     * 2。不可重复性：保证添加的元素按照equals()判断时，不能返回true即：相同的元素只能添加一个。
     *
     *二、添加加元素的过程：以HashSet为例 ：
     *      我们向HashSet中添加元素a，首先调用元素a所在类的hashCode()方法，计算元素a的哈希值
     *      此哈希值通过某种算法计算出在HashSet底层数组中的存放位置（即为：索引位置），判断
     *      数组此位置上是否已经有元素
     *            如果此位置上设有其他元素，则元素a添加成功。 --->情况1
    ...
    //LinkedHashSet的使用
    //LinkedHastSet作为HashSet的子类，在添加数据的同时，每个数据还维护两个引用，记录此数据前
    //数据和后一个数据。
